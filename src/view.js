...
function makeRenderer(canvas) {
  ...
  let lastCx = 0, lastCy = 0, lastOffsetX = 0, lastOffsetY = 0;

  const draw = function(simInstance, focusPersonId, realDt, rate, camX, camY) {
    ...
    const targetCamX = (focusPersonId >= 0 && focusPersonId < simInstance.pn)
      ? renderX[focusPersonId] - (VIEW_W >> 1)
      : (camX ?? (simInstance.W / 2)) - (VIEW_W >> 1);
    const targetCamY = (focusPersonId >= 0 && focusPersonId < simInstance.pn)
      ? renderY[focusPersonId] - (VIEW_H >> 1)
      : (camY ?? (simInstance.H / 2)) - (VIEW_H >> 1);
  ...
  draw.rx = renderX;
  draw.ry = renderY;
  draw.worldFromScreen = function(clientX, clientY, simInstance) {
    const rect = canvas.getBoundingClientRect();
    const scaleX = canvas.width / rect.width;
    const scaleY = canvas.height / rect.height;
    const canvasX = (clientX - rect.left) * scaleX - lastOffsetX;
    const canvasY = (clientY - rect.top) * scaleY - lastOffsetY;
    const wx = Math.floor(canvasX / TS) + lastCx;
    const wy = Math.floor(canvasY / TS) + lastCy;
    return { wx, wy, inside: wx >= 0 && wy >= 0 && wx < simInstance.W && wy < simInstance.H };
  };
  return draw;
}
...
function play(root, seed) {
  ...
  function possess(personId) {
    if (personId < 0 || personId >= S.pn || !S.palive[personId]) return;
    if (focus >= 0 && focus < S.pn) {
      S.pplanner[focus] = 0; // Previous body resumes autonomous AI
    }
    focus = personId;
    S.pplanner[focus] = 1; // Take direct control
    selIdx = -1;
    renderHands();
    journal(S, `You hopped into the mind of ${S.pname[focus]}.`);
  }

  function releaseToGhost() {
    if (focus >= 0 && focus < S.pn) {
      S.pplanner[focus] = 0; // Resume autonomous AI life
      camX = S.px[focus];
      camY = S.py[focus];
      journal(S, `You left ${S.pname[focus]} and ascended to Ghost Observer Mode.`);
    }
    focus = -1;
    selIdx = -1;
    renderHands();
  ...
  function move(d) { if (focus >= 0) inject(S, focus, { k: 'move', d }); }
  function hold(d) { if (focus >= 0) inject(S, focus, { k: 'hold', d }); }
  function release(d) { if (focus >= 0) inject(S, focus, { k: 'release', d }); }

  function actA() {
    release();
    if (panelMode) return panelConfirm();
    if (focus < 0) { openInhabitants(); return; }
  ...
  function startHold(d) {
    ...
    if (focus < 0) {
      const [dx, dy] = DIRS[d];
      camX = Math.max(0, Math.min(S.W - 1, camX + dx * 2));
      camY = Math.max(0, Math.min(S.H - 1, camY + dy * 2));
      return;
    }
    hold(d);
  }
  ...
  window.addEventListener('keydown', e => {
    ...
    if (e.code in KEYS) {
      e.preventDefault();
      if (panelMode) {
        if (!e.repeat) panelNav(KEYS[e.code]);
      } else if (focus < 0) {
        const [dx, dy] = DIRS[KEYS[e.code]];
        camX = Math.max(0, Math.min(S.W - 1, camX + dx));
        camY = Math.max(0, Math.min(S.H - 1, camY + dy));
      } else if (!e.repeat) {
        hold(KEYS[e.code]);
      }
    } else if (e.code === 'KeyP') {
      ...
      const eligible = dir.filter(p => p.id !== focus && p.kind === 0);
      if (eligible.length > 0) {
        possess(eligible[0].id);
      }
    } else if (e.code === 'KeyV') {
      e.preventDefault();
      panelMode === 'inhabitants' ? closePanel() : openInhabitants();
    } else if (e.code === 'KeyZ') {
      e.preventDefault();
      if (focus >= 0) {
      ...
      else if (focus >= 0 && e.code === 'Escape') releaseToGhost();
      else openMenu();
    } else if (/^Digit[1-7]$/.test(e.code)) {
      if (focus >= 0) selectSlot(+e.code[5] - 1);
    }
  });
  ...
  canvas.addEventListener('pointerdown', e => {
    ...
    dragStartCamX = (focus >= 0 && focus < S.pn) ? S.px[focus] : camX;
    dragStartCamY = (focus >= 0 && focus < S.pn) ? S.py[focus] : camY;
  });

  window.addEventListener('pointermove', e => {
    if (!isDragging) return;
    const dx = (e.clientX - dragStartX) / 16;
    const dy = (e.clientY - dragStartY) / 16;
    if (Math.abs(dx) > 0.5 || Math.abs(dy) > 0.5) {
      if (focus >= 0) releaseToGhost();
      camX = Math.max(0, Math.min(S.W - 1, dragStartCamX - dx));
      camY = Math.max(0, Math.min(S.H - 1, dragStartCamY - dy));
    }
  });
  ...
  window.addEventListener('pointerup', e => {
    ...
    if (dx < 6 && dy < 6 && !panelMode) {
      const { wx, wy, inside } = draw.worldFromScreen(e.clientX, e.clientY, S);
      if (inside) {
        const targetP = personAt(S, wx, wy) - 1;
        if (targetP >= 0 && S.palive[targetP]) {
          if (targetP !== focus) {
            possess(targetP);
          } else {
            view('me');
  ...
  function renderHands() {
    if (focus < 0) {
      ...
      handsSig = sig;
      handsEl.innerHTML = `<div class="slot" style="width:auto;padding:0 12px;cursor:pointer;color:#d9812b">👁️ Ghost Observer Mode · Click a villager or press P to possess · [V] Directory</div>`;
      handsEl.querySelector('.slot').addEventListener('pointerdown', openInhabitants);
      return;
    }
    const h = hands(S, focus);
  ...
  function openInhabitants() {
    ...
    panelItems = [
      ...
      { label: focus >= 0 ? 'Release to Ghost Observer Mode' : 'Close', go: () => { if (focus >= 0) releaseToGhost(); closePanel(); } }
    ];
    renderPanel('Inhabitants of the Valley', `${dir.filter(d => d.kind === 0).length} villagers · ${dir.filter(d => d.kind > 0).length} creatures`);
  }

  function openMenu() {
    panelMode = 'menu';
    panelSel = 0;
    const isAuto = focus >= 0 && S.pplanner[focus] === 0;
    panelItems = focus >= 0 ? [
      { label: 'What can I do here?', go: openActs },
      { label: 'Me', go: () => view('me') },
      { label: 'Inhabitants Directory', go: openInhabitants },
      ...
      { label: 'Release to Ghost Observer Mode', go: () => { releaseToGhost(); closePanel(); } },
      ...
      { label: 'Inhabitants Directory', go: openInhabitants },
      { label: 'Journal', go: () => view('journal') },
      { label: 'Arrive as a new wanderer on the road', go: () => {
        const roadY = S.H >> 1;
  ...
  function openDeathScreen(cause, text) {
    panelMode = 'death';
    panelSel = 0;
    const deadName = focus >= 0 ? S.pname[focus] : 'Character';
    const deadAge = focus >= 0 ? ageYears(S, focus) : 0;
    const home = focus >= 0 ? S.phome[focus] : -1;
  ...
  function updateActivePanel() {
    ...
    if (panelMode === 'me') {
      ...
      if (bars.length === 5 && me.needs) {
        for (let i = 0; i < 5; i++) {
          bars[i].style.width = (me.needs[i] | 0) + '%';
        }
      }
    }
    if (panelMode === 'journal' && S.journal.length !== lastJournalRendered) {
  ...
  function frame(now) {
    ...
    last = now;
    const waiting = focus >= 0 ? (S.pbusyUntil[focus] - S.time > 30) : false;
    const rate = waiting ? Math.max(speed, 600) : speed;
    step(S, S.time + real * rate);

    if (panelMode) release();
    draw(S, focus, real, rate, camX, camY);
    flushJournal();
    clock.textContent = 'day ' + (1 + dayOf(S.time)) + ' ' + hhmm(S.time) + (focus < 0 ? ' [ghost]' : S.pplanner[focus] === 0 ? ' [auto]' : '');
    updateActivePanel();
    ...
    if (focus >= 0) {
      ...
      hint.textContent = '👁️ Observer Mode · Click a person or press P to possess · [V] Directory';
      hint.style.opacity = !panelMode ? 1 : 0;
    }
  ...
  if (typeof window !== 'undefined') {
    window.sim = Object.assign(window.sim || {}, {
      S,
      getFocus: () => focus,
      setFocus: (id) => { possess(id); },
      possess: (id) => possess(id),
      releaseToGhost: () => releaseToGhost(),
      ...
      inspect: () => {
        const pState = focus >= 0 ? read(S, { person: focus }) : null;
        const availActs = focus >= 0 ? read(S, { acts: focus, slot: getSelectedThingId() }) : [];
        const facingTile = (focus >= 0) ? (() => {
        ...
        const directory = read(S, 'directory');
        return {
          time: S.time,
          day: 1 + dayOf(S.time),
          doy: 1 + doyOf(S.time),
