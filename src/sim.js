...
const DATA = {
  ...
  ACTS: [
    ...
    'forge', 'hire', 'pay', 'attack', 'bandage', 'flee', 'cast'
  ],
  ...
  ACT_MIN: {
    ...
    attack: 2, bandage: 5, flee: 1, cast: 30
  },
  ...
  VALUE: {
    grain: 1, berries: 1, flour: 2, bread: 4, log: 3, thatch: 2,
    knife: 20, spade: 15, axe: 40, waterskin: 8, penny: 1,
    ore: 2, iron: 6, cloth: 5, meat: 5, hide: 4, bow: 25
  }
};
...
const walkable = (sim, x, y) => DATA.TILE_WALK[tileAt(sim, x, y)] === 1 && !personAt(sim, x, y);
...
function tradeOffer(sim, sellerId, buyerId, stuffName, quantity) {
  ...
  if (lastDealTime !== undefined && sim.time - lastDealTime < 20 * 1440) {
    return { stuff: stuffName, qty: quantity, ask: 0, bid: 0, coin: 0, ok: false };
  }
  const ask = Math.max(1, Math.round(worthTo(sim, sellerId, stuffName) * quantity * 1.15));
  const bid = Math.round(worthTo(sim, buyerId, stuffName) * quantity);
  const buyerPennies = count(sim, 1, buyerId, 'penny');
  return { stuff: stuffName, qty: quantity, ask, bid, coin: buyerPennies, ok: bid >= ask && buyerPennies >= ask };
}
...
function evalNutrition(sim, personId) {
  ...
  if (food >= 40) return null;
  const foodInHand = held(sim, 1, personId, 'berries') >= 0 ? held(sim, 1, personId, 'berries') : held(sim, 1, personId, 'food');
  ...
  if (home >= 0) {
    ...
    const foodInStore = held(sim, 2, home, 'berries') >= 0 ? held(sim, 2, home, 'berries') : held(sim, 2, home, 'food');
    if (foodInStore >= 0) {
      return isAtHome
...
function evalSleepAndWarmth(sim, personId) {
  ...
  if (sleep < 25 || hourOfDay >= 21 || hourOfDay < 5 || (warmth < 35 && hasHut)) {
    if (hasHut && currentTile !== home) return { k: 'go', to: home };
    return { k: 'sleep' };
  }
...
function evalFoodProcessing(sim, personId, home, isAtHome, hasHut) {
  if (hasHut) {
    if (held(sim, 1, personId, 'flour') >= 0) return { k: 'act', slot: held(sim, 1, personId, 'flour'), act: 'bake' };
    if (isAtHome && count(sim, 2, home, 'grain') > 800 && count(sim, 2, home, 'bread') < 3) {
...
function evalMarketTrade(sim, personId, home, isAtHome, toHome, members) {
  const barnDays = (count(sim, 2, home, 'grain') * DATA.STUFF.grain.food + count(sim, 2, home, 'berries') * DATA.STUFF.berries.food) / (members.length * 50);
  ...
  if (sim.households.length > 1 && sim.season > 0.3) {
    if (barnDays > 500) {
      ...
      for (let q = 0; q < sim.pn; q++) {
        ...
        if (count(sim, 1, q, 'penny') < 3) continue;
        if (count(sim, 2, sim.phome[q], 'grain') > 400) continue;
      ...
      if (buyerId >= 0) {
        ...
        if (inHandGrain >= 0 && count(sim, 1, personId, 'grain') >= 20) {
        ...
        const storeGrain = held(sim, 2, home, 'grain');
      ...
      for (let q = 0; q < sim.pn; q++) {
        if (!sim.palive[q] || sim.phome[q] === home || sim.phome[q] < 0) continue;
        if (count(sim, 2, sim.phome[q], 'grain') < 600) continue;
        const d = Math.abs(sim.px[q] - sim.px[personId]) + Math.abs(sim.py[q] - sim.py[personId]);
        if (d < bestDist) {
          bestDist = d;
          sellerId = q;
      ...
      if (sellerId >= 0) {
        return goAct(sim, personId, idx(sim, sim.px[sellerId], sim.py[sellerId]), 'talk', -1);
      }
    }
  }
  return null;
...
function heuristic(sim, personId) {
  // 1. Critical Survival Evaluators
...
function ruleNeeds(sim, personId, deltaMinutes) {
  ...
  for (let needIndex = 0; needIndex < 5; needIndex++) {
    ...
    if (needIndex === 2) { // Warmth
      decayAmount = isInsideShelter ? -DATA.NEED_DECAY[2] * deltaMinutes * 2.0 : decayAmount * coldFactor;
    }
...
function applyAct(sim, personId, targetTile, action) {
  ...
  switch (action.act) {
    ...
    case 'sell': {
      ...
      if (offer.ok) doTrade(sim, other, personId, stuff, quantity, offer.ask);
      return actRes('talk');
    }
    case 'offer': {
      const other = personAt(sim, tx, ty) - 1;
...
function ruleArrivals(sim) {
  ...
  const settledCount = sim.households.filter(h => sim.tiles[h.home] === T.hut).length;
  if (!settledCount) return;
  if (sim.rng() > 0.02 * settledCount) return;

  const seedHome = sim.households[0].home;
